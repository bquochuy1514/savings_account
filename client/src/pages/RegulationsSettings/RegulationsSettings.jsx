  ...
  return (
    <div className="p-6 relative">
      ...
      <div className="flex justify-between items-start mb-8">
        ...
        />
        <Button icon={<LuPlus size={18} />} onClick={handleAdd} className="shadow-md cursor-pointer">
          Thêm quy định mới
        </Button>
      </div>
      ...
      {isLoading ? (
        <div className="flex flex-col justify-center items-center py-32 space-y-4">
          ...
          <p className="text-gray-400 font-medium animate-pulse">Đang đồng bộ dữ liệu...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {savingsTypes.length > 0 ? (
            savingsTypes.map((type) => (
              ...
              >
                <div className={`absolute top-0 left-0 w-full h-1.5 transition-colors ${type.isActive ? 'bg-gradient-to-r from-emerald-400 to-emerald-500' : 'bg-gray-300'}`}></div>
                
                <div className="flex justify-between items-start mb-5 mt-1">
                  <div>
                    <h3 className="text-xl font-extrabold text-gray-800 group-hover:text-blue-600 transition-colors">{type.name}</h3>
                    <p className="text-sm text-gray-500 mt-1 font-medium">
                      {type.termMonths === 0 ? 'Không kỳ hạn' : `Kỳ hạn: ${type.termMonths} tháng`}
                    </p>
                  </div>
                  <div className="flex flex-col items-end gap-2">
                    ...
                    <span className={`text-[10px] uppercase font-bold tracking-wider px-2 py-1 rounded-md ${type.isActive ? 'bg-emerald-50 text-emerald-600' : 'bg-gray-100 text-gray-500'}`}>
                      {type.isActive ? 'Đang áp dụng' : 'Tạm ngưng'}
                    </span>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 bg-gray-50/60 rounded-xl p-4 border border-gray-100 mb-5 flex-1">
                  <div>
                    <p className="text-[11px] text-gray-400 uppercase font-bold tracking-wider mb-0.5">Mở sổ tối thiểu</p>
                    <p className="font-semibold text-gray-800">{Number(type.minInitDeposit || 0).toLocaleString('vi-VN')} đ</p>
                  </div>
                  <div>
                    <p className="text-[11px] text-gray-400 uppercase font-bold tracking-wider mb-0.5">Gửi thêm tối thiểu</p>
                    <p className="font-semibold text-gray-800">{Number(type.minAddDeposit || 0).toLocaleString('vi-VN')} đ</p>
                  </div>
                  <div className="col-span-2 pt-3 mt-1 border-t border-gray-200/60">
                    <p className="text-[11px] text-gray-400 uppercase font-bold tracking-wider mb-0.5">Quy định rút tiền</p>
                    <p className="font-semibold text-gray-800">
                      {type.termMonths === 0 ? `Yêu cầu giữ sổ tối thiểu ${type.minWithdrawDays || 0} ngày` : 'Chỉ được rút tiền khi đến ngày đáo hạn'}
                    </p>
                  </div>
                </div>
            ...
            <div className="col-span-full bg-white border border-gray-200 border-dashed rounded-2xl p-16 text-center">
              ...
              <p className="text-gray-500">Bấm vào nút "Thêm quy định mới" để khởi tạo dữ liệu.</p>
            </div>
          )}
        </div>
      )}
