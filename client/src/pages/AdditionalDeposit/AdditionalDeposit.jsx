...
import AdditionalDepositReceiptModal from './AdditionalDepositReceiptModal'; 
...
export default function AdditionalDeposit() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post('/savings-book/deposit', payload);
      ...
      if (error.fieldErrors) {
        ...
        toast.error(error.message || 'Có lỗi xảy ra khi gửi tiền.');
      }
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="p-6 relative">
      <PageHeader
        ...
        badge="BM2"
        subtitle="Thực hiện gửi thêm tiền vào sổ tiết kiệm"
      />
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-2 gap-5 pl-1">
              <div>
                <Input
                  label={<>Số tiền gửi (VNĐ) <span className="text-red-400">*</span></>}
                  name="amount"
                ...
                />
                {selectedBook && selectedBook.savingsType?.minAddDeposit > 0 && (
                  <div className="mt-2.5 space-y-1.5">
                    <p className="text-xs text-blue-600 font-medium">* Số tiền gửi thêm tối thiểu: {Number(selectedBook.savingsType.minAddDeposit).toLocaleString('vi-VN')} VNĐ</p>
                  </div>
                )}
                {selectedBook && selectedBook.savingsType?.termMonths > 0 && (
                  <div className="mt-1.5 space-y-1.5">
                    <p className="text-xs text-orange-600 font-medium">* Sổ có kỳ hạn chỉ được gửi thêm đúng ngày đến hạn</p>
                  </div>
                )}
              </div>
